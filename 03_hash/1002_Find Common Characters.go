...
// 1. 26-slot frequency minima (recommended): keep the smallest count of each letter across all strings.
// 1. 26 计数数组取最小值：推荐；保留每个字母在所有字符串中出现次数的最小值。
...
// 2. Hash map minima: the same min-count idea, using a map instead of a 26-slot array.
// 2. 哈希表取最小值：思路相同，只是用 map 代替 26 格数组。
