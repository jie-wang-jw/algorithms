...
//
//  1. Sorting is required for directional pointer movement and deduplication.
//     排序是双指针定向移动和去重的前提。
//
//  2. i fixes the first number; three positions must remain after it.
//     i 固定第一个数，后面必须至少再留三个位置。
//
//  3. Pruning: nums[i] > target together with nums[i] >= 0 means the other three values are also
...
//     跳过重复的第一个数。
//
//  5. j fixes the second number; two positions must remain for left and right.
//     j 固定第二个数，后面必须给 left 和 right 各留一个位置。
//
//  6. The same pruning one level down: nums[i]+nums[j] >= 0 forces nums[j] >= 0,
//     so the remaining two values cannot decrease the sum.
//     同样的剪枝下移一层：nums[i]+nums[j] >= 0 说明 nums[j] >= 0，
//     剩下两个数都不小于它，无法把总和拉低。
//
//  7. Compare with the previous j only within the current i group.
//     只在当前 i 的范围内给 j 去重，因此条件是 j > i+1。
//
//  8. Search for the remaining two numbers in the range after j.
//     在 j 后面的区间寻找剩余两个数。
//
//  9. Record the quadruplet before moving past duplicate values.
//     先记录当前四元组，再移动指针跳过重复值。
//
//  10. Skip all copies of the current third and fourth values.
//     跳过第三、第四个数的所有重复值，避免重复答案。
//
//  11. Increase the sum by moving left to a larger value.
//     总和太小，left 右移以增大总和。
//
//  12. Decrease the sum by moving right to a smaller value.
//     总和太大，right 左移以减小总和。
...
//
// 步骤与要点 / Steps and notes:
//  1. Sorting keeps each answer ascending, which makes it usable as a deduplication key.
...
//  3. A repeated first number would only regenerate the same quadruplets.
//     重复的第一个数只会重新生成相同的四元组。
//  4. j = i+1 is this group's first choice, so deduplicate j only for j > i+1.
//     j = i+1 是本组的第一次选择，因此只在 j > i+1 时给 j 去重。
//  5. seen holds the values between j and the current k, which are candidates for c.
//     seen 保存位于 j 和当前 k 之间的数值，它们是 c 的候选。
//  6. Compute in int64 so the four-value sum cannot overflow a 32-bit int.
//     用 int64 计算，避免四数之和在 32 位 int 上溢出。
//  7. need was found strictly between j and k, so the four values are already ascending.
...
//  8. Insert after lookup so one position cannot supply both c and d.
