...
//  1. The map acts as a set; only key existence matters.
//     这里把 map 当作集合使用，只关心 key 是否存在。
//  2. Repeated values simply overwrite true, so the set remains unique.
//     重复数字只会再次写入 true，因此集合中仍然只有一份。
//  3. A true value means num appeared in nums1 and has not been used yet.
//     true 表示 num 在 nums1 中出现过，并且还没有加入结果。
//  4. Writing false instead of deleting the key has the same effect: the value is already used.
...
//
// 步骤与要点 / Steps and notes:
//  1. Sorting both sides lets a single forward scan compare values in increasing order.
//     两侧都排序后，就可以按数值递增的顺序单向扫描比较。
//  2. i walks nums1 and j walks nums2; neither pointer ever moves backward.
//     i 扫描 nums1，j 扫描 nums2，两个指针都不会回退。
//  3. The smaller value cannot match nums2[j] or anything larger after it, so drop it.
//     较小的值配不上 nums2[j]，也配不上它后面更大的值，直接丢弃。
//  4. Symmetric case: nums2[j] is too small to appear in the rest of nums1.
//     对称情况：nums2[j] 太小，不会出现在 nums1 的剩余部分中。
//  5. Equal values are part of the intersection, but each value may be recorded only once.
//     相等的值属于交集，但每个值只能记录一次。
//     Duplicates are adjacent after sorting, so comparing with the last output is enough.
//     排序后重复值相邻，因此只需要和上一个答案比较即可去重。
//  6. This value is settled on both sides, so advance both pointers.
