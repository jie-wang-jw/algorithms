...
n = len(s)。最坏时间 O(n)，每个字符检查一次；奇数长度可 O(1) 提前返回。辅助空间 O(n)，最坏情况下栈保存线性数量的期待右括号。
n = len(s). Worst-case time O(n), examining each character once; odd lengths return in O(1). Auxiliary space O(n) stores a linear number of expected closing brackets in the worst case.
*/
